...
public class ParentStudentRelationController extends BaseSecurityController
 { 
// POST    /v2/p/parent_student_relation/new/    controllers.business.ParentStudentRelationController.addParentStudentRelation(request: Request)
// GET     /v2/p/parent_student_relation/:id/    controllers.business.ParentStudentRelationController.getParentStudentRelation(request: Request,id:Long)
// POST    /v2/p/parent_student_relation/:id/    controllers.business.ParentStudentRelationController.updateParentStudentRelation(request: Request,id:Long)
// POST    /v2/p/parent_student_relation/        controllers.business.ParentStudentRelationController.deleteParentStudentRelation(request: Request)
// GET     /v2/p/parent_student_relation_list/   controllers.business.ParentStudentRelationController.listParentStudentRelation(request: Request,page:Int?=1,filter:String?="",status:Int?=0)
...
parentStudentRelation.setUpdateTime(currentTimeBySecond);parentStudentRelation.save(); 
return okJSON200();
});
}/**
...
ParentStudentRelation            newParentStudentRelation = Json.fromJson(jsonNode, ParentStudentRelation.class);
 ...
 if (newParentStudentRelation.parentId>0) originalParentStudentRelation.setParentId(newParentStudentRelation.parentId); 
 if (newParentStudentRelation.studentId>0) originalParentStudentRelation.setStudentId(newParentStudentRelation.studentId); 
 if (!ValidationUtil.isEmpty(newParentStudentRelation.relationship)) originalParentStudentRelation.setRelationship(newParentStudentRelation.relationship); 
 if (newParentStudentRelation.updateTime>0) originalParentStudentRelation.setUpdateTime(newParentStudentRelation.updateTime); 
long currentTimeBySecond = dateUtils.getCurrentTimeByMilliSecond();
originalParentStudentRelation.setUpdateTime(currentTimeBySecond); originalParentStudentRelation.save();
            return okJSON200();
        });
}
/**
